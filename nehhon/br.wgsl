  ...
  @binding(4) @group(0)  var<uniform> resolution : vec2<f32>;
  @binding(5) @group(0)  var<storage,read> circle : array<brnfo>;
...
fn vert_main(
 ...
 var vpos=pos*vec2<f32>(f32(mwidth*32));
  ...
  output.pos = vec4<f32>(2*(translation+vpos)/resolution, .75, 1);
  ...
  if(circle[0].rad>0){
...

    {
      ...
      if(d<r&&d>r-g) {
        ...
        var r=(1.0-abs(d-(r-g2))/(g2));
        color.c*=vec4(r*v+(1-r),1-r,1-r,0.0);
